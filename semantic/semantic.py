...
class Semantic:
    def __init__(self, symbol_table_reference):
        ...
        self.translation_routines = ("5", "10", "11", "16", "17", "22", "23", "24") #Rotinas que tem o Imprimir
    ...
    def declare_temporary(self):
        for i in range(self.Tx_count, -1, -1):# decrescente, até -1 q eh o valor de parada definido
    ...
    def run(self, prod_index, Alpha, validation, syntactic_error):
        # Definindo retorno padrão - Alpha lado esquerdo da regra
        ...
        # semantic_rules eh um dicionario
        ...
        # Testa a regra seleciona - A regra semantica altera o lado esquerdo
        if rule is not None:
            #rule eh uma funcao, parta debaixo do codigo (no python eh possiveladd uma funcao a uma variavel)
            non_terminal = rule(Alpha=tmp, #tmp = {"lexeme":str(Alpha), "token":str(Alpha), "type":"", "line":"", "column":""}
    ...
    # KWARGS - envia dicionario como parametros
    def create_semantic_rules(self):
        self.semantic_rules = {}

        # P -> P 
        def rule0(**kwargs):
            # a gente viu no slide q a tabela retornava o alpha
            return kwargs['Alpha']
        ...
        def rule5(**kwargs):
            ...
            # alterar tabela de simpbolos e add o tipo
